...
class FontColor(object):
    ...
    def sample_from_data(self, bg_mat):
        ...
        """
        bg_orig = bg_mat.copy()
        bg_mat = cv2.cvtColor(bg_mat, cv2.CV_RGB2Lab)
    ...
    def mean_color(self, arr):
        col = cv2.cvtColor(arr, cv2.CV_RGB2HSV)
        col = np.reshape(col, (np.prod(col.shape[:2]), 3))
        col = np.mean(col, axis=0).astype('uint8')
        return np.squeeze(cv2.cvtColor(col[None, None, :], cv2.CV_HSV2RGB))
    ...
    def complement(self, rgb_color):
        ...
        """
        col_hsv = np.squeeze(cv2.cvtColor(rgb_color[None, None, :], cv2.CV_RGB2HSV))
        col_hsv[0] = col_hsv[0] + 128  # uint8 mods to 255
        col_comp = np.squeeze(cv2.cvtColor(col_hsv[None, None, :], cv2.CV_HSV2RGB))
        return col_comp
    ...
    def triangle_color(self, col1, col2):
        ...
        col1 = np.squeeze(cv2.cvtColor(col1[None, None, :], cv2.CV_RGB2HSV))
        col2 = np.squeeze(cv2.cvtColor(col2[None, None, :], cv2.CV_RGB2HSV))
        h1, h2 = col1[0], col2[0]
        if h2 < h1: h1, h2 = h2, h1  # swap
        dh = h2 - h1
        if dh < 127: dh = 255 - dh
        col1[0] = h1 + dh / 2
        return np.squeeze(cv2.cvtColor(col1[None, None, :], cv2.CV_HSV2RGB))

    def change_value(self, col_rgb, v_std=50):
        col = np.squeeze(cv2.cvtColor(col_rgb[None, None, :], cv2.CV_RGB2HSV))
        x = col[2]
        vs = np.linspace(0, 1)
...
class Colorize(object):
    ...
    def color_border(self, col_text, col_bg):
        ...
        col_text = cv2.cvtColor(col_text, cv2.CV_RGB2HSV)
        ...
        if choice == 0:
            ...
            col_text = np.squeeze(cv2.cvtColor(col_text[None, None, :], cv2.CV_HSV2RGB))
        elif choice == 1:
            ...
            col_bg = cv2.cvtColor(col_bg, cv2.CV_RGB2HSV)
            col_bg = np.reshape(col_bg, (np.prod(col_bg.shape[:2]), 3))
            col_bg = np.mean(col_bg, axis=0).astype('uint8')
            col_bg = np.squeeze(cv2.cvtColor(col_bg[None, None, :], cv2.CV_HSV2RGB))
            col_text = np.squeeze(cv2.cvtColor(col_text[None, None, :], cv2.CV_HSV2RGB))
            col_text = self.font_color.triangle_color(col_text, col_bg)

        # now change the VALUE channel:        
        col_text = np.squeeze(cv2.cvtColor(col_text[None, None, :], cv2.CV_RGB2HSV))
        col_text[2] = get_sample(col_text[2])  # value
        return np.squeeze(cv2.cvtColor(col_text[None, None, :], cv2.CV_HSV2RGB))
    ...
    def check_perceptible(self, txt_mask, bg, txt_bg):
        ...
        bg = cv2.cvtColor(bg.copy(), cv2.CV_RGB2Lab)
        txt_bg = cv2.cvtColor(txt_bg.copy(), cv2.CV_RGB2Lab)
        bg_px = bg[txt_mask, :]
