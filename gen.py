...
def add_res_to_db(imgname, res, db):
    ...
    for i in xrange(ninstance):
        print
        colorize(Color.GREEN, 'added into the db %s ' % res[i]['txt'])
        ...
        print
        'type of res[i][\'txt\'] ', type(res[i]['txt'])

        # db['data'][dname].attrs['txt'] = res[i]['txt']
        db['data'][dname].attrs.create('txt', res[i]['txt'], dtype=h5py.special_dtype(vlen=unicode))
        print
        'type of db ', type(db['data'][dname].attrs['txt'])
        print
        colorize(Color.GREEN, 'successfully added')
        print
        res[i]['txt']
        print
        res[i]['img'].shape
        print
        'charBB', res[i]['charBB'].shape
        print
        'charBB', res[i]['charBB']
        print
        'wordBB', res[i]['wordBB'].shape
        print
        'wordBB', res[i]['wordBB']
        '''
...
def main(viz=False):
    # open databases:
    print
    colorize(Color.BLUE, 'getting data..', bold=True)
    db = get_data()
    print
    colorize(Color.BLUE, '\t-> done', bold=True)
    ...
    for i in xrange(start_idx, end_idx):
        ...
        try:
            ...
            print
            'depth shape,img shape', depth.shape, np.array(img).shape
            print
            'depth info', depth
            print
            'depth max min', np.max(depth), np.min(depth)
            # depth = depth[:,:,1]
            ...
            'H_channel', H.shape, H
            print
            'H_max min', np.max(H), np.min(H)
            print
            'scale', np.max(depth) / np.max(H)
            ...
            print
            'seg info', seg.shape, area.shape, label.shape
            # re-size uniformly:
            sz = depth.shape[:2][::-1]
            ...
            print
            'time consume in each pic', t2 - t1
            if viz:
            ...
            traceback.print_exc()
            print
            colorize(Color.GREEN, '>>>> CONTINUING....', bold=True)
            continue
    db.close()
    out_db.close()
