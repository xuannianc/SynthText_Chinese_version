...
NUM_IMAGES = -1  # no. of images to use for generation (-1 to use all available):
...
def main(viz=False):
    ...
    image_names = sorted(db['image'].keys())
    N = len(image_names)
    global NUM_IMAGES
    if NUM_IMAGES < 0:
        NUM_IMAGES = N
    start_idx, end_idx = 0, min(NUM_IMAGES, N)
    ...
    for i in range(start_idx, end_idx):
        ...
        try:
            # get the image:
            img = Image.fromarray(db['image'][image_name][:])
            # get the pre-computed depth:
            # there are 2 estimates of depth (represented as 2 "channels")
            # here we are using the second one (in some cases it might be
            # useful to use the other one):
            img_resize = img.resize(db['depth'][image_name].shape[:2])
            depth = db['depth'][image_name][:].T
            ...
            # get segmentation:
            seg = db['seg'][image_name][:].astype('float32')
            area = db['seg'][image_name].attrs['area']
            label = db['seg'][image_name].attrs['label']
            ...
            for ct in range(5):
                ...
                if len(res) > 0:
                    ...
                    add_res_to_db(image_name, res, out_db)
                    break
                else:
                    res = RV3.render_text(img, depth, seg, area, label,
                                          ninstance=INSTANCE_PER_IMAGE, viz=viz)
